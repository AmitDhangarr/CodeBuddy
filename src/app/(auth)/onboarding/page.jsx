...
function OnBoarding() {
  ...
  const handleSubmission = async (payload) => {
    ...
    try {
      ...
      if (!res.ok) {
        // 409 = duplicate email or handle, 400 = validation, 500 = server error
        if (res.status === 409) {
          setApiError(json.error || "An account with this email or handle already exists.");
        } else if (res.status === 400) {
          setApiError(json.error || "Invalid data. Please review your details and try again.");
        } else {
          setApiError(json.error || "Something went wrong on our end. Please try again shortly.");
        }
        setSubmitting(false);
        return;
      }
      ...
      // Redirect to sign-in after a short celebratory pause
      setTimeout(() => {
        router.push("/signin?account=created");
      }, 2800);
  ...
  const handleOnboardNext = () => {
    ...
    // Pull email + password from Zustand (set during the auth step before onboarding)
        ...
        <div className="fade-up" key={onboardStep}>
          ...
          {onboardStep === 2 && (
            <>
              ...
              {formData.skillsNeed.length > 0 && (
                <div style={{ fontSize: 11, color: T.text3, marginBottom: 10, display: "flex", alignItems: "center", gap: 5 }}>
                  <span style={{ fontSize: 13 }}>ℹ️</span>
                  Skills already selected as "needed" are unavailable here.
                </div>
              )}
              ...
              <div className="ob-skills-grid" style={{ display: "flex", flexWrap: "wrap", gap: 6, maxHeight: 220, overflowY: "auto" }}>
                {SKILLS_ALL
                  ...
                  .map(s => {
                    ...
                    const isDisabled = isConflict || isMaxed;

                    return (
                      <button
                        key={s}
                        className={`skill-chip ${isSelected ? "sel-have" : ""} ${isConflict && !isSelected ? "conflict" : ""}`}
                        onClick={() => !isConflict && toggleSkill("skillsHave", s)}
                        disabled={isDisabled}
                        title={isConflict ? "Already selected as a skill you need" : undefined}
                      >
                        {s}
                      </button>
                    );
          ...
          {onboardStep === 3 && (
            <>
              ...
              {formData.skillsHave.length > 0 && (
                <div style={{ fontSize: 11, color: T.text3, marginBottom: 10, display: "flex", alignItems: "center", gap: 5 }}>
                  <span style={{ fontSize: 13 }}>ℹ️</span>
                  Skills already selected as "have" are unavailable here.
                </div>
              )}
                ...
                {SKILLS_ALL
                  ...
                  .map(s => {
                    ...
                    return (
                      <button
                        key={s}
                        className={`skill-chip ${isSelected ? "sel-need" : ""} ${isConflict && !isSelected ? "conflict" : ""}`}
                        onClick={() => !isConflict && toggleSkill("skillsNeed", s)}
                        disabled={isDisabled}
                        title={isConflict ? "Already selected as a skill you have" : undefined}
                      >
                        {s}
                      </button>
                    );
          ...
          {onboardStep === 4 && (
            <>
              ...
              {/* ── Error banner: user already exists ── */}
              {apiError && (
                <div className="banner-error slide-down">
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: dark ? "#fca5a5" : "#b91c1c", lineHeight: 1.55 }}>
                      {apiError}
                    </div>
                    {apiError.toLowerCase().includes("already exists") && (
                      <button
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ marginTop: 10, height: 2, borderRadius: 99, background: dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)", overflow: "hidden" }}>
                      <div style={{
                        ...
                        background: "linear-gradient(90deg,#7de87d,#4ade80)",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 28 }}>
            {onboardStep > 0 ? (
              <button
                ...
                disabled={submitting || apiSuccess}
              >
                ← Back
              </button>
            ) : (
            ...
            {/* Hide the button after success — redirect is in flight */}
            {!apiSuccess && (
              <button
