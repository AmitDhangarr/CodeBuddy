...
import { AlertTriangle, Loader2, ArrowRight, ArrowLeft, Eye, EyeOff, Github} from "lucide-react";
...
const STATIC_CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .btn-primary{background:#7c3aed;border:1px solid #7c3aed;color:white;padding:11px 24px;border-radius:8px;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;transition:filter 0.15s ease;letter-spacing:-0.1px;display:inline-flex;align-items:center;justify-content:center;gap:8px}
  ...
  .social-btn{padding:10px;border-radius:8px;font-family:inherit;font-size:13px;font-weight:600;cursor:pointer;transition:border-color 0.15s ease,filter 0.15s ease;display:flex;align-items:center;justify-content:center;gap:9px;flex:1}
  ...
  .tab-indicator{position:absolute;top:4px;bottom:4px;border-radius:8px;transition:left 0.25s cubic-bezier(0.16,1,0.3,1),width 0.25s cubic-bezier(0.16,1,0.3,1)}
...
const ErrMsg = ({ msg }) =>
  msg ? <div style={{ fontSize: 11, color: "#f87171", marginTop: 5, display: "flex", alignItems: "center", gap: 5 }}><AlertTriangle style={iconSize(11, 12)} /> {msg}</div> : null;
  ...
  <div style={{ marginBottom: 18 }}>
    ...
    <div style={{ position: "relative" }}>
      ...
      {showToggle && (
        ...
        >
          {showPassword ? <EyeOff style={{ ...iconSize(15, 16), color: T.text }} /> : <Eye style={{ ...iconSize(15, 16), color: T.text }} />}
        </button>
      )}
    </div>
    <ErrMsg msg={error} />
    ...
    <div style={{ fontFamily: "'Inter',sans-serif", background: T.bg, color: T.text, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <nav style={{ padding: "0 clamp(16px,5vw,28px)", height: 58, display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${T.border}`, position: "sticky", top: 0, zIndex: 100, background: T.navBg, backdropFilter: "blur(20px)" }}>
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: 9, textDecoration: "none" }}>
          ...
          <span style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: "clamp(14px,4vw,16px)", letterSpacing: "-0.3px", color: T.text }}>CodeBuddy</span>
        </Link>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <button
            className="auth-back-btn"
            onClick={() => router.back()}
            style={{ background: "none", border: "none", cursor: "pointer", color: T.text3, fontSize: 13, fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}
          >
            <ArrowLeft style={iconSize(12, 13)} /> Back
          </button>
        </div>
      </nav>
        ...
        <div className="fade-up" style={{ width: "100%", maxWidth: 420 }}>
          ...
          <div className="card-flat" style={{ padding: 28, background: T.card, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <div key={authTab} className={slideClass}>
              <h2 style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: "clamp(20px,5vw,24px)", letterSpacing: "-1px", color: T.text, marginBottom: 6 }}>
              ...
              {authError && (
                <div style={{ fontSize: 12, color: "#f87171", marginBottom: 14, padding: "10px 14px", borderRadius: 8, background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.25)" }}>
                  {authError}
                </div>
              )}
              ...
              <form onSubmit={handleAuth} noValidate>
                ...
                <button type="submit" className="btn-primary" style={{ width: "100%", padding: 12 }} disabled={submitting}>
                  {submitting
                    ? <Loader2 className="spin" style={iconSize(14, 15)} />
                    : <>{authTab === "signin" ? "Sign in" : "Create account"} <ArrowRight style={iconSize(13, 14)} /></>
                  }
                </button>
              </form>
            </div>
