...
import { Search, Sparkles, Package, Settings, Check, Handshake, Wrench, Scale, Shuffle, Shield, Eye, Pencil, Trash2, Upload, Ban, PauseCircle, Clock, Mail } from "lucide-react";
...
export default function PrivacyPage() {
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .nav-link{font-size:14px;color:${T.text3};cursor:pointer;padding:8px 12px;border-radius:8px;transition:color 0.15s ease,background 0.15s ease,border-color 0.15s ease;text-decoration:none;display:flex;align-items:center;border-left:2px solid transparent}
  ...
  const sections = [
    {
      id: "overview", Icon: Search, title: "Overview",
      content: (
        <>
          ...
          <div style={{ background: T.aiBg, border: `1px solid ${T.aiBorder}`, borderRadius: 10, padding: "16px 18px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 10, fontWeight: 700, color: "#a78bfa", marginBottom: 6, letterSpacing: "1px", fontFamily: "'JetBrains Mono',monospace" }}><Sparkles style={iconSize(11, 12)} /> TL;DR</div>
    ...
    {
      id: "data-collected", Icon: Package, title: "Data We Collect",
      content: (
        <>
    ...
    {
      id: "how-we-use", Icon: Settings, title: "How We Use Your Data",
      content: (
        <>
          ...
          {["Generate skill-based compatibility matches between builders", "Personalise the AI match insight explanations you see", "Send account notifications and product updates (you can opt out anytime)", "Detect and prevent fraudulent or abusive activity", "Improve our matching algorithms using aggregated, anonymised signals", "Comply with legal obligations under applicable law"].map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 0" }}>
              <Check style={{ ...iconSize(12, 13), color: "#4ade80", marginTop: 2 }} strokeWidth={2.5} />
    ...
    {
      id: "sharing", Icon: Handshake, title: "Data Sharing",
      content: (
        <>
          <p style={{ fontSize: 14, color: T.text2, lineHeight: 1.8, marginBottom: 16 }}>
          ...
          {[
            { Icon: Wrench, label: "Service Providers", detail: "We use trusted third-party services (e.g. cloud hosting, email delivery, analytics) that are contractually bound to protect your data and may not use it for any other purpose." },
            { Icon: Scale, label: "Legal Requirements", detail: "We may disclose information when required by law, court order, or to protect the safety of our users or the integrity of the platform." },
            { Icon: Shuffle, label: "Business Transfers", detail: "In the unlikely event of a merger or acquisition, your data may transfer. We will notify you before your data is subject to a different privacy policy." },
          ].map((item, i) => (
            <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 10, padding: "16px 18px", marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                <item.Icon style={{ ...iconSize(15, 17), color: "#a78bfa" }} strokeWidth={1.75} />
      ...
      content: (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 10 }}>
            {[
              { Icon: Eye, right: "Access", desc: "Request a copy of all personal data we hold about you." },
              { Icon: Pencil, right: "Correction", desc: "Update or correct any inaccurate information." },
              { Icon: Trash2, right: "Deletion", desc: "Delete your account and all associated data permanently." },
              { Icon: Upload, right: "Portability", desc: "Export your profile and match data in a machine-readable format." },
              { Icon: Ban, right: "Objection", desc: "Object to processing your data for specific purposes." },
              { Icon: PauseCircle, right: "Restriction", desc: "Request we limit how we process your data." },
            ].map((item, i) => (
              <div key={i} style={{ background: T.surfaceA, border: `1px solid ${T.surfaceBorder}`, borderRadius: 10, padding: "14px 16px" }}>
                <item.Icon style={{ ...iconSize(18, 20), color: "#a78bfa", marginBottom: 8 }} strokeWidth={1.75} />
          ...
          <div className="sidebar" style={{ position: "sticky", top: 80, alignSelf: "start" }}>
            ...
            {sections.map((s, i) => (
              <a key={s.id} href={`#${s.id}`} className={`nav-link${activeSection === i ? " active" : ""}`} onClick={() => setActiveSection(i)}>
                <s.Icon style={{ ...iconSize(13, 14), marginRight: 8 }} strokeWidth={1.75} />{s.title}
              </a>
            ))}
            <div style={{ marginTop: 24, padding: "14px 16px", background: T.surfaceA, border: `1px solid ${T.surfaceBorder}`, borderRadius: 10 }}>
            ...
            {sections.map((s, i) => (
              <div key={s.id} id={s.id} className="section-block fade-up" style={{ animationDelay: `${i * 0.06}s` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                  <s.Icon style={{ ...iconSize(20, 22), color: "#a78bfa" }} strokeWidth={1.75} />
                  <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(20px,4vw,26px)", fontWeight: 700, color: T.text, letterSpacing: "-0.6px" }}>{s.title}</h2>
                </div>
                {s.content}
              </div>
            ))}
