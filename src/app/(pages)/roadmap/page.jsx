...
import { Zap, MessageSquare, User, Folder, Sparkles, BarChart3, Smartphone, Trophy, Github, Globe, Bot, GraduationCap, Network, DollarSign, Lightbulb, CheckCircle2, X, ChevronUp, ArrowRight } from "lucide-react";
...
export default function RoadmapPage() {
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .filter-btn{background:transparent;border:1px solid ${T.border};color:${T.text3};padding:6px 14px;border-radius:6px;font-family:inherit;font-size:12px;font-weight:600;cursor:pointer;transition:border-color 0.15s ease,color 0.15s ease,background 0.15s ease}
    ...
    .roadmap-card{background:${T.card};border:1px solid ${T.border};border-radius:10px;padding:22px;transition:border-color 0.15s ease,background 0.15s ease;cursor:default}
    .roadmap-card:hover{background:${T.cardHover};border-color:rgba(139,92,246,${dark ? "0.24" : "0.2"})}
    .vote-btn{background:transparent;border:1px solid ${T.border};border-radius:8px;padding:6px 12px;font-family:inherit;font-size:12px;font-weight:700;cursor:pointer;transition:border-color 0.15s ease,color 0.15s ease,background 0.15s ease;display:flex;align-items:center;gap:5px;color:${T.text3}}
    ...
    .modal-card{background:${T.bg2};border:1px solid ${T.border2};border-radius:14px;width:100%;max-width:480px;max-height:90vh;overflow-y:auto}
    .field-label{display:block;font-size:12px;font-weight:700;color:${T.text2};margin-bottom:7px;letter-spacing:0.2px}
    .field-input{width:100%;background:${dark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)"};border:1px solid ${T.border};border-radius:8px;padding:11px 14px;font-family:inherit;font-size:13px;color:${T.text};outline:none;transition:border-color 0.15s ease}
  ...
  const items = [
    { id: 1,  quarter: "Q1 2026", Icon: Zap,            title: "AI Matching Engine v1",       desc: "Core skill-based matching with complementary gap analysis. The foundation of CodeBuddy.",           status: "live",       category: "core",    votes: 312, tags: ["Matching", "AI"] },
    { id: 2,  quarter: "Q1 2026", Icon: MessageSquare,  title: "Real-Time Messaging",          desc: "In-app chat with match context always visible. No switching to Discord or Slack.",                    status: "live",       category: "core",    votes: 278, tags: ["Chat"] },
    { id: 3,  quarter: "Q1 2026", Icon: User,           title: "Builder Profiles",             desc: "Rich profiles with skill declarations, project links, availability, and collaboration intent.",       status: "live",       category: "core",    votes: 241, tags: ["Profiles"] },
    { id: 4,  quarter: "Q2 2026", Icon: Folder,         title: "Project Rooms",                desc: "Shared spaces for matched builders to plan tasks, set milestones, and track progress together.",    status: "inprogress", category: "collab",  votes: 198, tags: ["Collaboration", "Projects"] },
    { id: 5,  quarter: "Q2 2026", Icon: Sparkles,       title: "Match Insights v2",            desc: "Deeper AI explanations — personality signals, work style compatibility, and timezone overlap.",    status: "inprogress", category: "ai",      votes: 183, tags: ["AI", "Matching"] },
    { id: 6,  quarter: "Q2 2026", Icon: BarChart3,      title: "Analytics Dashboard",         desc: "See your match history, connection rate, and which skills are attracting the best builders.",       status: "inprogress", category: "pro",     votes: 142, tags: ["Pro", "Analytics"] },
    { id: 7,  quarter: "Q3 2026", Icon: Smartphone,     title: "Mobile App (iOS + Android)",  desc: "CodeBuddy in your pocket. Get match notifications and chat on the go.",                            status: "planned",    category: "platform", votes: 389, tags: ["Mobile"] },
    { id: 8,  quarter: "Q3 2026", Icon: Trophy,         title: "Builder Reputation Score",    desc: "A signal built from shipping history, ghost rate, and peer endorsements — not vanity metrics.",  status: "planned",    category: "core",    votes: 267, tags: ["Trust", "Reputation"] },
    { id: 9,  quarter: "Q3 2026", Icon: Github,         title: "GitHub Integration",          desc: "Connect your GitHub to auto-populate skills from your commit history and starred repos.",          status: "planned",    category: "integrations", votes: 231, tags: ["Integration"] },
    { id: 10, quarter: "Q4 2026", Icon: Globe,          title: "Multi-Language Support",      desc: "CodeBuddy in Hindi, Spanish, Mandarin, Arabic, Portuguese, and French.",                           status: "planned",    category: "platform", votes: 174, tags: ["Localization"] },
    { id: 11, quarter: "Q4 2026", Icon: Bot,            title: "AI Project Co-Pilot",         desc: "An AI assistant inside project rooms that helps plan sprints, debug decisions, and unblock work.", status: "exploring",  category: "ai",      votes: 456, tags: ["AI", "Pro"] },
    { id: 12, quarter: "Q4 2026", Icon: GraduationCap,  title: "Mentor Mode",                 desc: "A dedicated matching path for mentors and mentees with session scheduling and goal tracking.",     status: "exploring",  category: "collab",  votes: 312, tags: ["Mentorship"] },
    { id: 13, quarter: "2027+",   Icon: Network,        title: "Open API",                    desc: "Let builders build on CodeBuddy's matching graph. Public API with OAuth and rate limits.",         status: "exploring",  category: "platform", votes: 198, tags: ["API", "Developers"] },
    { id: 14, quarter: "2027+",   Icon: DollarSign,     title: "Builder Bounties",            desc: "Post a paid task or bounty visible to matched builders. Micro-contracts within the platform.",     status: "exploring",  category: "collab",  votes: 267, tags: ["Monetization"] },
  ];
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: s.bg, border: `1px solid ${s.border}`, borderRadius: 6, padding: "3px 10px" }}>
        {status === "live" && <span style={{ width: 5, height: 5, borderRadius: "50%", background: s.color, animation: "pulse 2s ease-in-out infinite" }} />}
        <span style={{ fontSize: 10, fontWeight: 700, color: s.color, letterSpacing: "0.3px", fontFamily: "'JetBrains Mono',monospace" }}>{s.label}</span>
      </span>
    );
  };
    ...
    <div style={{ fontFamily: "'Inter',sans-serif", background: T.bg, color: T.text, minHeight: "100vh" }}>
      ...
      <nav style={{ background: T.navBg, backdropFilter: "blur(28px)", borderBottom: `1px solid ${T.border}`, position: "sticky", top: 0, zIndex: 100, padding: "0 clamp(16px,5vw,32px)", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
          <Logo />
          <span style={{ fontFamily: "'Inter',sans-serif", fontWeight: 600, fontSize: 18, letterSpacing: "-0.4px", color: T.text }}>CodeBuddy</span>
        </Link>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1060, margin: "0 auto", padding: "clamp(32px,6vw,64px) clamp(16px,5vw,32px)" }}>
        ...
        <div className="fade-up" style={{ marginBottom: 44 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: T.surfaceA, border: `1px solid ${T.surfaceBorder}`, borderRadius: 6, padding: "4px 13px", marginBottom: 18 }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: "#a78bfa", letterSpacing: "0.8px", textTransform: "uppercase", fontFamily: "'JetBrains Mono',monospace" }}>Roadmap</span>
          </div>
          <h1 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(32px,7vw,52px)", fontWeight: 700, color: T.text, letterSpacing: "-1.6px", lineHeight: 1.08, marginBottom: 14 }}>
            What we're <span style={{ color: "#a78bfa" }}>building</span>
          </h1>
        ...
        <div className="fade-up" style={{ display: "flex", gap: 10, marginBottom: 36, flexWrap: "wrap" }}>
          {stats.map((s, i) => (
            <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 10, padding: "12px 18px", display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, fontSize: 24, color: s.color, letterSpacing: "-1px" }}>{s.v}</span>
              <span style={{ fontSize: 12, color: T.text3, fontWeight: 600 }}>{s.l}</span>
            </div>
          ))}
          <div style={{ background: T.aiBg, border: `1px solid ${T.aiBorder}`, borderRadius: 10, padding: "12px 18px", display: "flex", alignItems: "center", gap: 8, marginLeft: "auto" }}>
            <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 10, fontWeight: 700, color: "#a78bfa", fontFamily: "'JetBrains Mono',monospace" }}><Sparkles style={iconSize(11, 12)} /> Vote to shape priorities</span>
          </div>
        </div>
          ...
          return (
            <div key={q} style={{ marginBottom: 52 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 22 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, background: isCurrentQ ? "rgba(124,58,237,0.12)" : T.card, border: `1px solid ${isCurrentQ ? "rgba(124,58,237,0.35)" : T.border}`, borderRadius: 6, padding: "6px 16px" }}>
                  {isCurrentQ && <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#fb923c", animation: "pulse 2s ease-in-out infinite" }} />}
                  <span style={{ fontSize: 12, fontWeight: 700, color: isCurrentQ ? "#fb923c" : T.text3, letterSpacing: "0.5px", textTransform: "uppercase", fontFamily: "'JetBrains Mono',monospace" }}>
                  ...
                  <div key={item.id} className="roadmap-card fade-up" style={{ animationDelay: `${i * 0.06}s` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, gap: 10 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <item.Icon style={{ ...iconSize(19, 21), color: "#a78bfa" }} strokeWidth={1.75} />
                        <StatusBadge status={item.status} />
                      </div>
                      <button className={`vote-btn${voted[item.id] ? " voted" : ""}`} onClick={() => handleVote(item.id)}>
                        <ChevronUp style={iconSize(12, 13)} strokeWidth={voted[item.id] ? 3 : 2} />
                        <span style={{ fontFamily: "'JetBrains Mono',monospace" }}>{item.votes + (voted[item.id] ? 1 : 0)}</span>
                      </button>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                      {item.tags.map(tag => (
                        <span key={tag} style={{ fontSize: 10, color: T.text3, background: dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)", border: `1px solid ${T.border}`, borderRadius: 6, padding: "2px 8px", fontWeight: 600, fontFamily: "'JetBrains Mono',monospace" }}>{tag}</span>
                      ))}
                    </div>
                  </div>
                ))}
        ...
        <div className="fade-up" style={{ background: T.surfaceA, border: `1px solid ${T.surfaceBorder}`, borderRadius: 14, padding: "clamp(28px,6vw,44px)", textAlign: "center", marginTop: 20 }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 14 }}>
            <Lightbulb style={{ ...iconSize(28, 32), color: "#a78bfa" }} strokeWidth={1.75} />
          </div>
          <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(22px,5vw,34px)", fontWeight: 700, color: T.text, letterSpacing: "-0.9px", marginBottom: 12 }}>
            Have an idea?
          </h2>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
            <button className="btn-primary" style={{ padding: "11px 24px" }} onClick={openModal}>Submit a feature request <ArrowRight style={iconSize(13, 14)} /></button>
            <Link href="/changelog">
            ...
            {formState === "success" ? (
              <div style={{ padding: "44px 32px", textAlign: "center" }}>
                <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 18px" }}>
                  <CheckCircle2 style={{ ...iconSize(22, 24), color: "#22c55e" }} strokeWidth={2} />
                </div>
                <h3 style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: 24, color: T.text, marginBottom: 10, letterSpacing: "-0.6px" }}>Request submitted</h3>
              ...
              <form onSubmit={handleSubmit} noValidate>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "26px 28px 0" }}>
                  <div>
                    <h3 style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: 22, color: T.text, letterSpacing: "-0.6px", marginBottom: 6 }}>Submit a feature request</h3>
                    <p style={{ fontSize: 12, color: T.text2, lineHeight: 1.6 }}>Goes straight to our product team's backlog for review.</p>
                  </div>
                  <button type="button" className="btn-icon" onClick={closeModal} aria-label="Close" style={{ flexShrink: 0 }}><X style={iconSize(13, 14)} /></button>
                </div>
                ...
                <div style={{ padding: "22px 28px 28px", display: "flex", flexDirection: "column", gap: 18 }}>
                  ...
                  {formState === "error" && (
                    <div style={{ background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.25)", borderRadius: 8, padding: "11px 14px", fontSize: 12, color: "#f87171", fontWeight: 600 }}>
