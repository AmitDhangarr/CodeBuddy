...
import { Wrench, Sparkles, Globe, BookOpen, Rocket, Lightbulb, FlaskConical, PartyPopper, DollarSign, Heart, ArrowRight } from "lucide-react";
...
const Avatar = ({ hue, initials, size = 56, radius = 10, dark }) => (
  <div style={{ width: size, height: size, borderRadius: radius, background: hsla(hue, 70, 60, dark ? 0.14 : 0.1), border: `1.5px solid ${hsla(hue, 70, 60, 0.3)}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.3, fontWeight: 700, color: hsl(hue), flexShrink: 0, fontFamily: "'JetBrains Mono',monospace" }}>
    {initials}
  </div>
);
...
export default function About() {
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .card:hover{background:${T.cardHover};border-color:rgba(139,92,246,${dark ? "0.24" : "0.2"})}
  ...
  const VALUES = [
    { Icon: Wrench, title: "Builders first", desc: "Every decision starts with one question: does this help someone ship faster? We're developers ourselves, and we build for ourselves." },
    { Icon: Sparkles, title: "Honest algorithms", desc: "Our matching is explainable. We never hide why two people were paired. Transparency is a feature, not an afterthought." },
    { Icon: Globe, title: "Global, local", desc: "CodeBuddy started in India but belongs to every developer on Earth. Timezone-aware, language-sensitive, globally ambitious." },
    { Icon: Sparkles, title: "Complement, don't compete", desc: "We believe the best products are built by people with different skills who trust each other — not solo heroes or homogeneous teams." },
    { Icon: BookOpen, title: "Learn in public", desc: "We share our metrics, our mistakes, and our roadmap. A community that watches us grow holds us accountable." },
    { Icon: Rocket, title: "Ship or nothing", desc: "Features that don't ship don't exist. We move fast, break less, and celebrate every launch — big or tiny." },
  ];

  const MILESTONES = [
    { year: "Jan 2024", event: "CodeBuddy idea born after two engineers couldn't find each other online", Icon: Lightbulb },
    { year: "Apr 2024", event: "Private beta launched with 80 hand-picked builders from Twitter/X", Icon: FlaskConical },
    { year: "Jul 2024", event: "AI matching engine v1 ships — 40% higher connection rate overnight", Icon: Sparkles },
    { year: "Oct 2024", event: "1,000 active users milestone. First 10 projects shipped by matched pairs", Icon: PartyPopper },
    { year: "Jan 2025", event: "Seed round closed. Team grows to 8. Mobile app enters beta", Icon: DollarSign },
    { year: "Now 2026", event: "3,200+ builders, 40+ countries, 847 shipped projects and counting", Icon: Rocket },
  ];
  ...
  const SectionHead = ({ eyebrow, title, sub }) => (
    <div style={{ textAlign: "center", marginBottom: 52 }}>
      ...
      <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(28px,6vw,42px)", fontWeight: 700, color: T.text, letterSpacing: "-1.4px", lineHeight: 1.08, marginBottom: sub ? 14 : 0 }}>{title}</h2>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "clamp(60px,12vw,110px) clamp(16px,5vw,32px) clamp(40px,8vw,72px)", textAlign: "center" }}>
          <div className="fade-up">
            <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: T.surfaceA, border: `1px solid ${T.surfaceBorder}`, borderRadius: 6, padding: "4px 13px", marginBottom: 24 }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: "#a78bfa", letterSpacing: "0.8px", textTransform: "uppercase", fontFamily: "'JetBrains Mono',monospace" }}>Our Story</span>
            </div>
            <h1 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(36px,9vw,68px)", fontWeight: 700, lineHeight: 1.05, letterSpacing: "-1.6px", color: T.text, marginBottom: 24, maxWidth: 720, margin: "0 auto 24px" }}>
              Built by builders,<br /><span style={{ color: "#a78bfa" }}>for builders</span>
            </h1>
          ...
          <div className="hero-stats fade-up" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, maxWidth: 760, margin: "0 auto", animationDelay: "0.15s" }}>
            {[
              ...
              <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 10, padding: "18px 12px", textAlign: "center" }}>
                <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "clamp(22px,5vw,32px)", fontWeight: 700, color: T.text, letterSpacing: "-1px" }}>{s.v}</div>
        ...
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(50px,10vw,80px)" }}>
          <div style={{ background: T.aiBg, border: `1px solid ${T.aiBorder}`, borderRadius: 14, padding: "clamp(32px,6vw,56px) clamp(24px,5vw,56px)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative", maxWidth: 620 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#a78bfa", letterSpacing: "1px", textTransform: "uppercase", marginBottom: 16, fontFamily: "'JetBrains Mono',monospace" }}>Our Mission</div>
              <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(24px,5vw,38px)", fontWeight: 700, color: T.text, letterSpacing: "-1.1px", lineHeight: 1.12, marginBottom: 20 }}>
                Make developer collaboration as easy as<br /><span style={{ color: "#a78bfa" }}>writing a function.</span>
              </h2>
        ...
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(50px,10vw,80px)" }}>
          <SectionHead eyebrow="Values" title={<>What we<br /><span style={{ color: "#a78bfa" }}>believe</span></>} sub="Six principles that guide every product decision we make." />
          <div className="values-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
            {VALUES.map((v, i) => (
              <div key={i} className="card fade-up" style={{ padding: "24px 22px", animationDelay: `${i * 0.07}s` }}>
                <v.Icon style={{ ...iconSize(22, 26), color: "#a78bfa", marginBottom: 14 }} strokeWidth={1.75} />
        ...
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(50px,10vw,80px)" }}>
          <SectionHead eyebrow="Timeline" title={<>How we got<br /><span style={{ color: "#a78bfa" }}>here</span></>} />
          <div style={{ maxWidth: 640, margin: "0 auto", position: "relative" }}>
            ...
            {MILESTONES.map((m, i) => (
              <div key={i} className="fade-up" style={{ display: "flex", gap: 24, marginBottom: 28, animationDelay: `${i * 0.1}s` }}>
                <div style={{ width: 40, height: 40, borderRadius: 8, background: T.surfaceA, border: `1px solid ${T.surfaceBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, zIndex: 1 }}>
                  <m.Icon style={{ ...iconSize(16, 18), color: "#a78bfa" }} strokeWidth={1.75} />
                </div>
                <div style={{ paddingTop: 8 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "#a78bfa", letterSpacing: "0.8px", textTransform: "uppercase", marginBottom: 5, fontFamily: "'JetBrains Mono',monospace" }}>{m.year}</div>
        ...
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(50px,10vw,80px)" }}>
          <SectionHead eyebrow="Team" title={<>The builders<br /><span style={{ color: "#a78bfa" }}>behind CodeBuddy</span></>} sub="A small team that moves fast, ships constantly, and genuinely cares about developers." />
            ...
            {TEAM.map((t, i) => (
              <div key={i} className="card fade-up" style={{ padding: "24px", animationDelay: `${i * 0.08}s`, textAlign: "center" }}>
                <div style={{ display: "flex", justifyContent: "center", marginBottom: 14 }}>
                  <Avatar hue={t.hue} initials={t.initials} size={60} radius={10} dark={dark} />
                </div>
        ...
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(60px,12vw,100px)" }}>
          <div style={{ background: dark ? "rgba(124,58,237,0.07)" : "rgba(124,58,237,0.05)", border: "1px solid rgba(124,58,237,0.18)", borderRadius: 14, padding: "clamp(36px,8vw,60px) clamp(24px,6vw,44px)", textAlign: "center", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative" }}>
              <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(26px,6vw,42px)", fontWeight: 700, color: T.text, letterSpacing: "-1.4px", lineHeight: 1.1, marginBottom: 14 }}>
                Join the builders<br /><span style={{ color: "#a78bfa" }}>already shipping.</span>
              </h2>
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
                <Link href="/signup"><button className="btn-primary" style={{ padding: "13px 30px", fontSize: 14 }}>Create your profile <ArrowRight style={iconSize(13, 14)} /></button></Link>
