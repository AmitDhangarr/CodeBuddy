...
function makeTheme(dark) {
  ...
  return {
    bg: "#f5f4fb", bg2: "#ffffff",
    border: "rgba(0,0,0,0.07)", border2: "rgba(0,0,0,0.12)",
    text: "#18172b", text2: "#5a5580", text3: "#a09ac0",
    card: "rgba(255,255,255,0.85)", cardHover: "rgba(255,255,255,1)",
    input: "rgba(0,0,0,0.03)", inputBorder: "rgba(0,0,0,0.09)",
    shadow: "0 24px 64px rgba(0,0,0,0.10)",
    navBg: "rgba(245,244,251,0.93)",
...
.plan-card{background:${T.card};border:1px solid ${T.border};border-radius:22px;padding:28px;transition:all .3s;position:relative;overflow:hidden}
.plan-card:hover{border-color:rgba(124,58,237,.3);transform:translateY(-4px);box-shadow:${T.shadow}}
...
.btn-ghost{background:transparent;border:1px solid ${T.border2};color:${T.text2};padding:13px 24px;border-radius:11px;font-family:inherit;font-size:14px;font-weight:600;cursor:pointer;transition:all .2s;width:100%}
.btn-ghost:hover{border-color:${T.text2};color:${T.text}}
.btn-sm-ghost{background:transparent;border:1px solid ${T.border2};color:${T.text2};padding:8px 16px;border-radius:9px;font-family:inherit;font-size:12px;font-weight:600;cursor:pointer;transition:all .2s}
...
.faq-item{border:1px solid ${T.border};border-radius:14px;overflow:hidden;margin-bottom:10px;transition:border-color .2s}
...
.modal-box{background:${T.modalBg};border:1px solid ${T.border2};border-radius:26px;width:min(520px,calc(100vw - 32px));box-shadow:0 32px 80px rgba(0,0,0,.8);overflow:hidden;animation:scaleIn .3s cubic-bezier(.16,1,.3,1) both;max-height:90vh;overflow-y:auto}

.pay-method-tab{flex:1;padding:9px 4px;border:none;background:transparent;color:${T.text2};font-size:11px;font-weight:700;cursor:pointer;border-radius:9px;transition:all .2s;font-family:inherit;text-align:center;white-space:nowrap}
...
.pay-input{width:100%;background:${T.input};border:1.5px solid ${T.inputBorder};border-radius:11px;padding:11px 14px;font-size:14px;color:${T.text};font-family:inherit;outline:none;transition:border-color .2s,box-shadow .2s}
...
.pay-label{font-size:10px;font-weight:700;color:${T.text3};letter-spacing:.7px;text-transform:uppercase;margin-bottom:6px}

.upi-tile{background:${T.card};border:1.5px solid ${T.border};border-radius:13px;padding:10px 6px;text-align:center;cursor:pointer;transition:all .2s;font-size:10px;font-weight:700;color:${T.text2}}
...
.bank-row{background:${T.card};border:1.5px solid ${T.border};border-radius:11px;padding:10px 14px;display:flex;align-items:center;gap:10px;cursor:pointer;transition:all .2s;font-size:12px;color:${T.text2};font-weight:600}
...
.wallet-tile{background:${T.card};border:1.5px solid ${T.border};border-radius:13px;padding:12px 8px;text-align:center;cursor:pointer;transition:all .2s;font-size:11px;font-weight:700;color:${T.text2}}
...
.emi-tile{background:${T.card};border:1.5px solid ${T.border};border-radius:12px;padding:12px;cursor:pointer;transition:all .2s;text-align:center}
...
.proc-row{display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:9px;transition:all .35s;font-size:12px;color:${T.text3}}
...
.rcpt-row{display:flex;justify-content:space-between;font-size:12px;padding:6px 0;border-bottom:1px solid ${T.border};color:${T.text2}}
      ...
      <div style={{ padding: "22px 28px 18px", borderBottom: `1px solid ${T.border}` }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button onClick={onBack} style={{ background: T.card, border: `1px solid ${T.border2}`, color: T.text2, width: 30, height: 30, borderRadius: 8, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
            <div>
      ...
      <div style={{ padding: "20px 28px 8px" }}>
        ...
        {method === "card" && (
          <div>
            ...
            <div style={{ background: T.card, border: `1px solid ${T.border2}`, borderRadius: 10, padding: "10px 14px", fontSize: 11, color: T.text3, marginBottom: 4 }}>
        ...
        {method === "wallet" && (
          <div>
            ...
            <div style={{ fontSize: 11, color: T.text3, background: T.card, borderRadius: 9, padding: "10px 13px", marginBottom: 4 }}>
  ...
  return (
    <div style={{ padding: "36px 28px", textAlign: "center" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 14, fontSize: 10, color: T.text3, flexWrap: "wrap" }}>
        <span>🔒 SSL encrypted</span><span>🛡 Razorpay PCI-DSS</span><span>🏦 RBI compliant</span>
      </div>
    </div>
  )
}
...
function StepSuccess({ plan, yearly, payData, onClose, T }) {
  ...
  return (
    <div style={{ padding: "32px 28px 28px", textAlign: "center" }}>
      ...
      <h2 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 28, fontWeight: 400, color: T.text, letterSpacing: "-.5px", marginBottom: 6 }}>Payment successful!</h2>
      <p style={{ fontSize: 14, color: T.text2, marginBottom: 22 }}>
        Welcome to CodeBuddy <span style={{ color: plan.color, fontWeight: 700 }}>{plan.name}</span> 🎉
      </p>
      <div style={{ background: T.card, border: `1px solid ${T.border2}`, borderRadius: 14, padding: 16, marginBottom: 20, textAlign: "left" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 12px" }}>
          {Object.entries(plan.features).filter(([, v]) => v !== false).map(([k, v]) => (
            <div key={k} style={{ fontSize: 11, color: T.text2, display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ color: "#4ade80", fontSize: 10 }}>✓</span>{typeof v === "string" ? v : k}
            </div>
          ))}
        </div>
      </div>
      <button className="btn-primary" onClick={onClose} style={{ fontSize: 15, marginBottom: 10 }}>Go to Dashboard →</button>
      <button onClick={onClose} style={{ background: "none", border: "none", color: T.text3, fontSize: 12, cursor: "pointer", fontFamily: "inherit", width: "100%", padding: "6px 0" }}>Close</button>
      ...
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        {!["processing", "success"].includes(step) && (
          <div style={{ height: 3, background: T.border }}>
            <div style={{ height: "100%", borderRadius: 99, transition: "width .4s ease", background: "linear-gradient(90deg,#7c3aed,#a78bfa)", width: step === "proceed" ? "33%" : step === "method" ? "66%" : "100%" }} />
          </div>
        )}
...
export default function PricingPage() {
  ...
  return (
    <div style={{ fontFamily: "'Instrument Sans',sans-serif", background: T.bg, color: T.text, minHeight: "100vh", transition: "background .3s, color .3s" }}>
      <style>{CSS}</style>
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
        <div style={{ position: "absolute", top: "-18%", left: "-8%", width: 650, height: 650, borderRadius: "50%", background: dark ? "radial-gradient(circle,hsla(259,70%,35%,0.11) 0%,transparent 65%)" : "radial-gradient(circle,hsla(259,70%,80%,0.12) 0%,transparent 65%)" }} />
        <div style={{ position: "absolute", bottom: "-12%", right: "-8%", width: 550, height: 550, borderRadius: "50%", background: dark ? "radial-gradient(circle,hsla(300,60%,30%,0.07) 0%,transparent 65%)" : "radial-gradient(circle,hsla(300,60%,70%,0.08) 0%,transparent 65%)" }} />
        <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: dark ? 0.018 : 0.04 }} xmlns="http://www.w3.org/2000/svg">
          <defs><pattern id="g" width="60" height="60" patternUnits="userSpaceOnUse"><path d="M 60 0 L 0 0 0 60" fill="none" stroke={dark ? "white" : "#7c3aed"} strokeWidth="0.5" /></pattern></defs>
          <rect width="100%" height="100%" fill="url(#g)" />
        </svg>
      </div>

      {/* Nav */}
      <nav style={{ position: "sticky", top: 0, zIndex: 100, padding: "0 clamp(16px,5vw,32px)", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", background: T.navBg, backdropFilter: "blur(10px)", borderBottom: `1px solid ${T.border}`, transition: "background .3s, border-color .3s" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <Logo />
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <section style={{ maxWidth: 800, margin: "0 auto", padding: "clamp(50px,10vw,96px) clamp(16px,5vw,32px) 0", textAlign: "center" }}>
          ...
          <div className="fade-up" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 14, marginBottom: 52, animationDelay: ".15s" }}>
            ...
            <button onClick={() => setYearly(p => !p)} style={{ width: 52, height: 28, borderRadius: 14, border: "none", cursor: "pointer", background: yearly ? "#7c3aed" : T.toggleTrackOff, position: "relative", transition: "background .3s" }}>
        ...
        <section style={{ maxWidth: 1100, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) 80px" }}>
          <div className="plans-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }}>
            {PLANS.map((plan, i) => (
              <div key={plan.id} className={`plan-card fade-up${plan.id === "pro" ? " featured" : ""}`} style={{ animationDelay: `${i * 0.08}s` }}>
                ...
                <button className={plan.ctaStyle === "primary" ? "btn-primary" : "btn-ghost"} onClick={() => handlePlanClick(plan)} style={{ textAlign: "center" }}>
                  {plan.cta}
                </button>
              </div>
            ))}
