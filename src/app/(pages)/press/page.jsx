...
import { Palette, Camera, Ruler, Users, CheckCircle2, Quote, Newspaper, ArrowRight, Download, Heart } from "lucide-react";
...
const ASSETS = [
  { name: "Logo Pack (SVG + PNG)", size: "2.4 MB", Icon: Palette, desc: "Dark, light, and icon-only variants in all standard sizes.", url: "https://your-cdn.com/assets/codebuddy-logo-pack.zip" },
  { name: "Product Screenshots", size: "8.1 MB", Icon: Camera, desc: "High-res screenshots of the matching dashboard, profile, and chat.", url: "https://your-cdn.com/assets/codebuddy-screenshots.zip" },
  { name: "Brand Guidelines", size: "1.1 MB", Icon: Ruler, desc: "Color palette, typography, usage rules, and do-not-use examples.", url: "https://your-cdn.com/assets/codebuddy-brand-guidelines.pdf" },
  { name: "Founder Photos", size: "4.7 MB", Icon: Users, desc: "Hi-res headshots of Vikram Anand and Nisha Kapoor.", url: "https://your-cdn.com/assets/codebuddy-founder-photos.zip" },
];
 ...
 const { dark, toggleDark } = useThemeStore();
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .coverage-card{background:${T.card};border:1px solid ${T.border};border-radius:10px;padding:22px;transition:border-color 0.15s ease,background 0.15s ease;cursor:pointer;text-decoration:none;display:block}
    .coverage-card:hover{background:${T.cardHover};border-color:rgba(124,58,237,0.24)}
    .asset-card{background:${T.card};border:1px solid ${T.border};border-radius:10px;padding:20px;transition:border-color 0.15s ease,background 0.15s ease;display:flex;align-items:center;gap:16px}
    .asset-card:hover{background:${T.cardHover};border-color:rgba(124,58,237,0.24)}
    .boilerplate-box{background:${T.card};border:1px solid ${T.border};border-radius:10px;padding:22px;font-size:13px;color:${T.text2};line-height:1.75;position:relative}
    .copy-btn{position:absolute;top:14px;right:14px;background:${T.surfaceA};border:1px solid ${T.surfaceBorder};color:#a78bfa;padding:5px 12px;border-radius:8px;font-family:inherit;font-size:11px;font-weight:700;cursor:pointer;transition:background 0.15s ease;display:inline-flex;align-items:center;gap:5px}
    .copy-btn:hover{background:rgba(124,58,237,0.15)}
    .quote-card{background:${T.card};border:1px solid ${T.border};border-radius:10px;padding:24px;transition:border-color 0.15s ease,background 0.15s ease}
    .quote-card:hover{border-color:rgba(124,58,237,0.22);background:${dark ? "rgba(124,58,237,0.04)" : "rgba(124,58,237,0.02)"}}
    ...
    <div style={{ textAlign: "center", marginBottom: 48 }}>
      {eyebrow && (
        <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: T.surfaceA, border: `1px solid ${T.surfaceBorder}`, borderRadius: 6, padding: "4px 13px", marginBottom: 18 }}>
          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, fontWeight: 700, color: "#a78bfa", letterSpacing: "0.8px", textTransform: "uppercase" }}>{eyebrow}</span>
        </div>
      )}
      <h2 style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: "clamp(26px,6vw,40px)", color: T.text, letterSpacing: "-1px", lineHeight: 1.08, marginBottom: sub ? 14 : 0 }}>{title}</h2>
  ...
  const TypeBadge = ({ type, hue }) => (
    <span style={{ fontFamily: "'JetBrains Mono',monospace", padding: "3px 9px", borderRadius: 6, fontSize: 10, fontWeight: 700, background: hsla(hue, 70, 60, dark ? 0.12 : 0.09), border: `1px solid ${hsla(hue, 70, 60, 0.28)}`, color: hsl(hue) }}>
      {type}
    </span>
  );
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "clamp(60px,12vw,110px) clamp(16px,5vw,32px) clamp(36px,7vw,60px)", textAlign: "center" }}>
          <div className="fade-up">
            <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: T.surfaceA, border: `1px solid ${T.surfaceBorder}`, borderRadius: 6, padding: "4px 13px", marginBottom: 22 }}>
              <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, fontWeight: 700, color: "#a78bfa", letterSpacing: "0.8px", textTransform: "uppercase" }}>Press & Media</span>
            </div>
            <h1 style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: "clamp(34px,9vw,62px)", lineHeight: 1.06, letterSpacing: "-1.8px", color: T.text, marginBottom: 20 }}>
              Everything you need<br /><span style={{ color: "#a78bfa" }}>to cover CodeBuddy.</span>
            </h1>
            ...
            <a href="mailto:press@codebuddy.dev">
              <button className="btn-primary" style={{ padding: "12px 28px", fontSize: 14 }}><BtnLabel>Contact press team</BtnLabel></button>
            </a>
          </div>
        </section>
        ...
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(44px,9vw,72px)" }}>
          <div className="stats-row fade-up" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, animationDelay: "0.1s" }}>
            {STATS.map((s, i) => (
              <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 10, padding: "22px 16px", textAlign: "center" }}>
                <div style={{ fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, fontSize: "clamp(24px,5vw,36px)", color: T.text, letterSpacing: "-1px", lineHeight: 1 }}>{s.v}</div>
        ...
        {/* ── BOILERPLATE ── */}
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(44px,9vw,72px)" }}>
          <SectionHead eyebrow="About CodeBuddy" title={<>Standard<br /><span style={{ color: "#a78bfa" }}>boilerplate</span></>} sub="Use this text in articles and publications. You may shorten it; please don't alter factual claims." />
          <div className="boilerplate-box" style={{ maxWidth: 700, margin: "0 auto" }}>
            <button className="copy-btn" onClick={() => handleCopy(boilerplate)}>
              {copied && <CheckCircle2 style={iconSize(11, 13)} />}
        ...
        {/* ── PRESS QUOTES ── */}
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(44px,9vw,72px)" }}>
          <SectionHead eyebrow="What they're saying" title={<>Quotes from<br /><span style={{ color: "#a78bfa" }}>the press</span></>} />
          <div className="quotes-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
            {QUOTES.map((q, i) => (
              <div key={i} className="quote-card fade-up" style={{ animationDelay: `${i * 0.08}s` }}>
                <Quote style={{ ...iconSize(26, 32), color: "#7c3aed", marginBottom: 12, opacity: 0.4 }} fill="#7c3aed" />
                <p style={{ fontSize: 13, color: T.text2, lineHeight: 1.72, marginBottom: 16 }}>{q.quote}</p>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: hsla(q.hue, 70, 60, dark ? 0.14 : 0.1), border: `1px solid ${hsla(q.hue, 70, 60, 0.28)}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Newspaper style={iconSize(13, 15)} color={hsl(q.hue)} />
                  </div>
        ...
        {/* ── PRESS COVERAGE ── */}
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(44px,9vw,72px)" }}>
          <SectionHead eyebrow="Coverage" title={<>CodeBuddy in<br /><span style={{ color: "#a78bfa" }}>the news</span></>} sub="Recent mentions and features across the developer and startup press." />
            ...
            {COVERAGE.map((c, i) => (
              <a key={i} href={c.url} target="_blank" rel="noopener noreferrer" className="coverage-card fade-up" style={{ animationDelay: `${i * 0.07}s` }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                  ...
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: T.text3 }}>{c.date}</span>
                </div>
                <p style={{ fontSize: 13, color: T.text2, lineHeight: 1.6 }}>"{c.headline}"</p>
                <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 4, color: "#a78bfa", fontSize: 12, fontWeight: 600 }}>
                  <BtnLabel min={12} max={14}>Read article</BtnLabel>
                </div>
              </a>
            ))}
          </div>
        ...
        {/* ── BRAND ASSETS ── */}
        <section style={{ maxWidth: 1060, margin: "0 auto", padding: "0 clamp(16px,5vw,32px) clamp(44px,9vw,72px)" }}>
          <SectionHead eyebrow="Brand Kit" title={<>Download our<br /><span style={{ color: "#a78bfa" }}>brand assets</span></>} sub="All assets are pre-cleared for press use. Please follow our brand guidelines." />
          <div className="assets-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 12 }}>
            {ASSETS.map((a, i) => (
              <div key={i} className="asset-card fade-up" style={{ animationDelay: `${i * 0.08}s` }}>
                <div style={{ width: 48, height: 48, borderRadius: 10, background: T.surfaceA, border: `1px solid ${T.surfaceBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <a.Icon style={iconSize(20, 22)} color="#a78bfa" />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: T.text3 }}>{a.size}</span>
                  {downloadError === i && (
                ...
                >
                  {downloadingIdx === i ? "Downloading…" : <BtnLabel Icon={Download} min={12} max={14}>Download</BtnLabel>}
                </button>
              </div>
            ))}
          </div>
          ...
          <div className="press-contact-grid" style={{ background: T.aiBg, border: `1px solid ${T.aiBorder}`, borderRadius: 10, padding: "clamp(32px,6vw,52px)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40, alignItems: "center" }}>
            <div>
              ...
              <h2 style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: "clamp(22px,4vw,32px)", color: T.text, letterSpacing: "-0.5px", lineHeight: 1.15, marginBottom: 14 }}>
                Need something<br /><span style={{ color: "#a78bfa" }}>specific?</span>
              </h2>
              ...
              <a href="mailto:press@codebuddy.dev">
                <button className="btn-primary" style={{ padding: "11px 24px" }}><BtnLabel>press@codebuddy.dev</BtnLabel></button>
              </a>
            </div>
        ...
        <footer style={{ borderTop: `1px solid ${T.border}`, padding: "clamp(28px,5vw,44px) clamp(16px,5vw,32px)", maxWidth: 1060, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            ...
            <span style={{ fontSize: 12, color: T.text3, display: "inline-flex", alignItems: "center", gap: 4 }}>© 2026 CodeBuddy · Made with <Heart style={iconSize(11, 13)} fill="#f87171" color="#f87171" /> in India</span>
          </div>
        </footer>
      </div>
    </div>
