 ...
 const { dark, toggleDark } = useThemeStore();
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .post-card{background:${T.card};border:1px solid ${T.border};border-radius:18px;padding:24px;transition:all 0.28s;cursor:pointer;display:flex;flex-direction:column}
    ...
    .article-body h2{font-family:'Instrument Serif',serif;font-weight:400;font-size:clamp(22px,4vw,28px);color:${T.text};letter-spacing:-0.5px;margin:38px 0 16px}
    ...
    return (
      <div style={{ fontFamily: "'Instrument Sans',sans-serif", background: T.bg, color: T.text, minHeight: "100vh" }}>
        ...
        <nav className="landing-nav" style={{ position: "sticky", top: 0, zIndex: 100, padding: "0 clamp(16px,5vw,32px)", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button className="btn-ghost" onClick={closePost}>← All posts</button>
        </nav>

        <div style={{ position: "relative", zIndex: 1, maxWidth: 720, margin: "0 auto", padding: "clamp(40px,8vw,72px) clamp(16px,5vw,32px) 0" }}>
          <div className="fade-up">
            <button className="btn-ghost" onClick={closePost} style={{ marginBottom: 28, fontSize: 12, padding: "7px 14px" }}>← Back to all posts</button>

            <div style={{ marginBottom: 18 }}>
              <TagBadge tag={p.tag} hue={p.tagHue} />
            </div>

            <h1 style={{ fontFamily: "'Instrument Serif',serif", fontSize: "clamp(28px,6vw,46px)", fontWeight: 400, letterSpacing: "-1.2px", lineHeight: 1.1, color: T.text, marginBottom: 26 }}>
              {p.title}
            </h1>
        ...
        <section style={{ position: "relative", zIndex: 1, maxWidth: 1060, margin: "0 auto", padding: "20px clamp(16px,5vw,32px) clamp(60px,10vw,90px)" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: T.text3, textTransform: "uppercase", letterSpacing: "0.6px", marginBottom: 16 }}>Keep reading</div>
          <div className="posts-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 14 }}>
            {fallbackRelated.map(rp => (
              <div key={rp.id} className="post-card" onClick={() => openPost(rp)}>
                ...
                <h3 style={{ fontFamily: "'Instrument Serif',serif", fontSize: "clamp(16px,3vw,19px)", fontWeight: 400, color: T.text, letterSpacing: "-0.4px", lineHeight: 1.25, marginBottom: 10 }}>{rp.title}</h3>
            ...
            <div className="fade-up" style={{ background: dark ? "rgba(124,58,237,0.06)" : "rgba(124,58,237,0.04)", border: `1px solid rgba(124,58,237,0.18)`, borderRadius: 22, padding: "clamp(28px,5vw,44px)", cursor: "pointer", transition: "all 0.28s", position: "relative", overflow: "hidden" }}
              ...
              <div className="featured-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40, alignItems: "center", position: "relative" }}>
                <div>
                  ...
                  <button className="btn-primary" style={{ padding: "10px 22px", fontSize: 13 }} onClick={(e) => { e.stopPropagation(); openPost(featured); }}>Read post →</button>
                </div>
            ...
            <div style={{ position: "relative" }}>
              ...
              {subState === "success" ? (
                <div style={{ padding: "8px 0" }}>
                  <div style={{ width: 48, height: 48, borderRadius: "50%", background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 22 }}>✓</div>
                  <h2 style={{ fontFamily: "'Instrument Serif',serif", fontSize: "clamp(20px,4vw,28px)", color: T.text, letterSpacing: "-0.6px", marginBottom: 10 }}>You're subscribed</h2>
                ...
                <>
                  ...
                  <p style={{ marginTop: 14, fontSize: 11, color: T.text3 }}>Join {1400 + subscribedEmails.length}+ developers already subscribed.</p>
                </>
              )}
            </div>
          </div>
