...
export default function OtherProfileTab({
  ...
  // Safe border fallback so undefined T.border never produces "1px solid undefined"
            ...
            {loading ? (
              ...
              <>
                <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                  <div style={{ fontFamily:"'Instrument Serif',serif", fontSize:24, color:text, letterSpacing:"-0.5px" }}>
                ...
                <div style={{ display:"flex", gap:7, marginTop:12, flexWrap:"wrap" }}>
                  ...
                  <span style={{ padding:"3px 10px", borderRadius:99, fontSize:11, fontWeight:600, background:skillNeedBg, border:`1px solid ${skillNeedBorder}`, color:skillNeedText }}>
          ...
          {!loading && (
            connectionStatus === "connected" ? (
              <button onClick={handleMessage} style={{
                ...
                color:text2, padding:"7px 16px", borderRadius:10,
                cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:600, flexShrink:0,
              }}>💬 Message</button>
            ) : connectionStatus === "pending" ? (
              <button disabled style={{
                background:"transparent", border:`1px solid ${border}`,
                color:text3, padding:"7px 16px", borderRadius:10,
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", marginTop:22, paddingTop:18, borderTop:`1px solid ${border}` }}>
          {[
            ...
            <div key={i} style={{ textAlign:"center", borderRight: i < 3 ? `1px solid ${border}` : "none" }}>
              <div style={{ fontFamily:"'Instrument Serif',serif", fontSize:22, color:text }}>{s.v}</div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginTop:10 }}>
            {loading
              ...
              : (Array.isArray(profile?.skills_have) ? profile.skills_have : []).map(s => (
                <span key={s} style={{ padding:"3px 10px", borderRadius:99, fontSize:11, fontWeight:600, background:skillHaveBg, border:`1px solid ${skillHaveBorder}`, color:skillHaveText }}>{s}</span>
              ))
            }
          </div>
        </div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginTop:10 }}>
            {loading
              ...
              : (Array.isArray(profile?.skills_need) ? profile.skills_need : []).map(s => (
                <span key={s} style={{ padding:"3px 10px", borderRadius:99, fontSize:11, fontWeight:600, background:skillNeedBg, border:`1px solid ${skillNeedBorder}`, color:skillNeedText }}>{s}</span>
              ))
            }
          </div>
        </div>
                ...
                <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4, height:"100%" }}>
                  <div style={{ flex:1, width:"100%", background: dark ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.04)", borderRadius:6, overflow:"hidden", display:"flex", alignItems:"flex-end" }}>
                    <div style={{
                      ...
                      background: (d?.count ?? 0) === 0 ? "transparent" : "linear-gradient(180deg,#a855f7,#7c3aed)",
                      borderRadius:4,
                      transition:"height .5s cubic-bezier(.34,1.56,.64,1)",
                      transitionDelay:`${i*60}ms`,
                    }} />
                  </div>
                  <span style={{ fontSize:9, color:text3, fontWeight:700 }}>{d?.label ?? ""}</span>
                </div>
              ))
          }
        </div>
      ...
      <div className="card-flat" style={{ padding:16, marginBottom:14 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
          ...
          <div style={{ display:"flex", gap:14 }}>
            ...
            <div style={{ textAlign:"right" }}>
              <div style={{ fontFamily:"'Instrument Serif',serif", fontSize:18, color:text }}>{loading ? "—" : streak.filter(c => (c?.count ?? 0) > 0).length}</div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:4, marginBottom:4 }}>
          {["M","T","W","T","F","S","S"].map((d,i) => (
            <div key={i} style={{ fontSize:9, color:text3, textAlign:"center" }}>{d}</div>
          ))}
        </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:5, marginTop:10, justifyContent:"flex-end" }}>
          <span style={{ fontSize:10, color:text3 }}>Less</span>
          {[0,1,2,3,4].map(c => (
            <div key={c} style={{ width:10, height:10, borderRadius:2, background:heatColor(c) }} />
          ))}
          <span style={{ fontSize:10, color:text3 }}>More</span>
        </div>
      </div>
      ...
      <div className="card-flat" style={{ padding:20 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            {loading
              ? Array.from({ length:2 }).map((_,i) => (
                  <div key={i} style={{ padding:16, borderRadius:13, border:`1px solid ${border}`, background: dark ? "rgba(255,255,255,.02)" : "rgba(0,0,0,.02)" }}>
                    <Sk w="60%" h={16} mb={10} />
                    <Sk w="100%" h={12} mb={5} />
                    <Sk w="80%"  h={12} mb={12} />
                      ...
                      >
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8, gap:8, flexWrap:"wrap" }}>
                          ...
                          <div style={{ display:"flex", gap:6, alignItems:"center", flexShrink:0 }}>
                            ...
                            <span style={{ fontSize:11, color:text3 }}>★ {p?.stars ?? 0}</span>
                        ...
                        {(p?.tags ?? []).length > 0 && (
                          <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                            {(p.tags ?? []).map(tag => (
                              <span key={tag} style={{ padding:"2px 9px", borderRadius:99, fontSize:10, fontWeight:600, background: dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.05)", color:text2 }}>{tag}</span>
                            ))}
                          </div>
                        )}
                      </div>
        ...
        {profileTab === "endorsements" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            {loading
              ? Array.from({ length:2 }).map((_,i) => (
                  <div key={i} style={{ padding:16, borderRadius:13, border:`1px solid ${border}`, display:"flex", gap:14 }}>
                    <Sk w="38px" h={38} r={10} />
                    <div style={{ flex:1 }}>
                      <Sk w="50%" h={13} mb={8} />
                      <Sk w="100%" h={12} mb={4} />
              ...
              : endorsements.length === 0
                ...
                : endorsements.map((e, i) => {
                    ...
                    return (
                      <div key={i} style={{ padding:16, background: dark ? "rgba(255,255,255,.02)" : "rgba(0,0,0,.02)", borderRadius:13, border:`1px solid ${border}`, display:"flex", gap:14, alignItems:"flex-start" }}>
                        ...
                        <div style={{ flex:1 }}>
                          <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:6, flexWrap:"wrap" }}>
                            ...
                            <span style={{ fontSize:11, fontWeight:700, padding:"2px 9px", borderRadius:99, background:skillHaveBg, border:`1px solid ${skillHaveBorder}`, color:skillHaveText }}>{e?.skill ?? ""}</span>
                          </div>
                          {e?.note && (
            ...
            {connectionStatus === "connected" && (
              <button className="endorse-btn" style={{
                ...
                color:text3, borderRadius:13, cursor:"pointer", fontFamily:"inherit",
            ...
            {loading
              ...
              : (Array.isArray(profile?.activity_feed) ? profile.activity_feed : []).length === 0
                ...
                : (profile?.activity_feed ?? []).map((a, i) => {
                    ...
                    return (
                      <div key={i} style={{ display:"flex", gap:12, alignItems:"center", padding:"10px 13px", borderRadius:11, background: dark ? "rgba(255,255,255,.02)" : "rgba(0,0,0,.02)" }}>
                        <div style={{ width:32, height:32, borderRadius:9, flexShrink:0, background:`hsla(${hue},70%,60%,${bgAlpha})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 }}>
                          {a?.icon ?? ""}
                        </div>
                        ...
                        <span style={{ fontSize:10, color:text3 }}>{a?.time ?? ""}</span>
                      </div>
                    );
                  })
            }
        ...
        {profileTab === "connections" && (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            {loading
              ? Array.from({ length:4 }).map((_,i) => (
                  <div key={i} style={{ display:"flex", gap:12, alignItems:"center", padding:"12px 14px", borderRadius:13, border:`1px solid ${border}`, background: dark ? "rgba(255,255,255,.02)" : "rgba(0,0,0,.02)" }}>
                    <Sk w="36px" h={36} r={10} />
              ...
              : mutuals.length === 0
                ...
                : mutuals.map((m, i) => {
                    ...
                    const lightL = dark ? 75 : 45;
                    return (
                      <div key={m?.id ?? i} style={{ display:"flex", gap:12, alignItems:"center", padding:"12px 14px", borderRadius:13, border:`1px solid ${border}`, background: dark ? "rgba(255,255,255,.02)" : "rgba(0,0,0,.02)" }}>
                        <div style={{ width:36, height:36, borderRadius:10, flexShrink:0, background:`hsla(${hue},60%,60%,${dark ? 0.15 : 0.1})`, border:`1px solid hsla(${hue},60%,60%,.3)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700, color:`hsl(${hue},55%,${lightL}%)` }}>
                          {nameInitials(m?.name ?? "")}
                        </div>
                        <div style={{ flex:1, minWidth:0 }}>
